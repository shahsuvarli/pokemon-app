...
class App extends React.Component {
  ...
  startGame() {
    ...
    this.setState({
      ...
      winner: this.state.t1sum>this.state.t2sum ? 'Team1' : 'Team2'
    });
  }
    ...
    return (
      <div className="App">
        <div className="main">
          ...
          <div className="board">
            <TeamBoard players={this.state.team1} number={1} team="Team 1" />
            <button onClick={this.startGame}>START</button>
            <TeamBoard players={this.state.team2} number={2} team="Team 2" />
          </div>
          Winner: {this.state.winner} ({`this.state.${this.state.winner}`})
          {/* {this.state.t1sum > this.state.t2sum
            ? `Winner is Team1 with ${this.state.t1sum} points`
            : "no"} */}
        </div>
      </div>
    );
